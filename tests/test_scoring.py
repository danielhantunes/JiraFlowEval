...
def test_metric_value_numeric_clamped():
    """Numeric metrics are clamped to 0-5."""
    assert metric_value(3, "code_quality") == 3.0
    assert metric_value(10, "readme_clarity") == 5.0
...
def test_compute_final_score_all_max():
    ...
    score = compute_final_score(metrics, weights, max_score=10.0)
    assert score == 10.0


def test_compute_final_score_mixed():
    """Mixed metrics produce scaled score."""
    metrics = {
        "pipeline_runs": True,
        "gold_generated": True,
